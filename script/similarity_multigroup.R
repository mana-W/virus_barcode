...
# input 2 is a table of input files
...
args=commandArgs(T)
file <- read.csv(args[2],header=T)
clone_1<-read.table(file$Earlier[1],header=T,stringsAsFactors = F)
clone_2<-read.table(file$Later[1],header=T,stringsAsFactors = F)
clone_1$group <- file$group[1] ; clone_2$group <- file$group[1]


for(i in c(2:length(file$group))){
  clone1_sub <- read.table(file$Earlier[i],header=T,stringsAsFactors = F)
  clone1_sub$group <- file$group[i]
  clone2_sub <- read.table(file$Later[i],header=T,stringsAsFactors = F)
  clone2_sub$group <- file$group[i]
  clone_1 <- rbind(clone_1,clone1_sub)
  clone_2 <- rbind(clone_2,clone2_sub)
  rm(clone1_sub,clone2_sub)
}

clone<-rbind(clone_1,clone_2)
clone$Virus.BC <- unlist(apply(clone,1,function(x){
  x <- as.character(x)
  vb <- strsplit(x[2],";")[[1]]
  vb <- paste0(paste0(vb, "-",x[5]),collapse = ";")
  return(vb)
}))
...
clone$group <- paste0(clone$time,"_",clone$group)
jac_sample<-list()
for(t in c(1:500)){
  for(i in names(table(clone$group))){
    clone$clone_sample[clone$group==i]<-sample(clone$clone[clone$group==i],length(clone$clone[clone$group==i]))
  }
